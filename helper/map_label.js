...
module.exports.map = async function(labels, provider, cb){
    if(provider==="gmail") {
        return await handleGmail(labels, cb);
    }
};
...
async function handleGmail(labels, cb) {
    ...
    cb(labels.map((x,i )=> [x, translated[i]]))
