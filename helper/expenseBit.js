...
// const senderEmailData = require('../models/senderEmailInfo');
...
class ExpenseBit {
    ...
    */
    static async MoveMailFromInBOX(user_id, auth, from_email, label) {
        let mail = await email.find({ "from_email": { $in: from_email}, "user_id": user_id }).catch(err => { console.error(err.message, err.stack); });
        let mail_array = mail.map(x => x._id);
        let mailList = await emailInformation.find({ "from_email_id":{$in:mail_array}}, { "email_id": 1 }).catch(err => { console.error(err.message, err.stack); });
        ...
        if (mailList) {
            ...
            email.updateMany(oldvalue, newvalues, { upsert: true }).catch(err => {
    ...
    */
    ...
    static async createEmailInfoAndStore(user_id, mail) {
        let emailInfo = {};
        emailInfo['user_id'] = user_id;
        emailInfo['mail_data'] = null;
        emailInfo['email_id'] = mail.id;
    ...
    */
    static async checkEmail(emailObj, mail, user_id, auth) {
        ...
        // else{
        //     await ExpenseBit.createEmailInfoAndStore(user_id, mail);
        // }
    }
    ...
    static async checkEmailNew(emailObj, mail, user_id, auth) {
        ...
        if (emailInfo.from_email.toLowerCase().indexOf('@gmail') != -1) {
            console.log(emailInfo.from_email,"hdhdgh")
            return
        } else if (emailInfo) {
            let emailInfoNew = {};
            emailInfoNew['email_id'] = emailInfo['email_id'];
