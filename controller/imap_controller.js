...
let MyEvent = com.anoop.event.MyEvent;
...
router.post('/loginWithImap', async (req, res) => {
    try {
        ...
        let response = await Controller.login(req.body.username.toLowerCase(), req.body.password, profile).catch(err => {
            ...
            MyEvent.sentryCaptureException(err, {tags: {email_domain: req.body.username.split("@")[1], pass_length:  req.body.password.length}});
